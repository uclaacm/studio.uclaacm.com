...
function CopyButton({ onClick: onClickProp, tooltipDuration }: CopyButtonProps){
	...
	return (
		<Tooltip title="Copied" open={tooltipOpen} arrow>
			<IconButton size="small" aria-label="Copy code"
				onClick={onClick}
				sx={theme => ({
					position: "absolute",
			...
			><IsaxIcon name="isax-copy"/></IconButton>
		</Tooltip>
	)
}
...
export default function Highlight({ children, language, block }: HighlightProps){
	...
	const Container = block ?
		({ children }) => <Typography variant="body1" component="pre" sx={{ position: "relative" }}>{children}</Typography> :
		({ children }) => <>{children}</>
	...
	return <Container>
		{ block && <CopyButton onClick={() => { copyToClipboard(); }} />}
