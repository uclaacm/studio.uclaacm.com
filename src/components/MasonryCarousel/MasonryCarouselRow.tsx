...
export default React.forwardRef<HTMLDivElement, MasonryCarouselRowProps>(
  function MasonryCarouselRow(props: MasonryCarouselRowProps, ref) {
    ...
    const [virtualCellOffsets, setVirtualCellOffsets] = React.useState(
      Array.from<number>({ length: nVirtualizedCells }).fill(0),
    );
    ...
    virtualCellInViews.forEach((inView, i) => {
      React.useEffect(() => {
        ...
        // new box comes in frame from left
          ...
          (leftMostVCIndex + nVirtualizedCells - 1) % nVirtualizedCells === i &&
          inView
        ) {
    ...
    React.useEffect(() => {
      ...
      const leftBox =
        (leftMostVCIndex + nVirtualizedCells - 1) % nVirtualizedCells;

      // leftBox
      if (leftMostVCIndex === 0) {
        setVirtualCellOffsets([
          ...Array.from<number>({ length: nVirtualizedCells - 1 }).fill(
            curOffset,
          ),
          curOffset - 1,
        ]);
        ...
        setVirtualCellOffsets(
          Array.from({ length: nVirtualizedCells }).map((_, i) =>
            i >= leftBox ? curOffset : curOffset + 1,
          ),
        );
      }
    }, [leftMostVCIndex]);
    ...
    return (
      ...
      >
        {virtualCellRefs.map((ref, i) => {
          const cellIndex =
            (((i + nVirtualizedCells * virtualCellOffsets[i]) % nCells) +
              nCells) %
            nCells;
