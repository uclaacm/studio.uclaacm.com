...
import {
  Axis,
  GamepadAxisChangeEvent,
  GamepadJoystickChangeEvent,
  Joystick,
} from "~/util/gamepad";
...
function NavBarRadialButton({
  uniqueKey,
  icon,
  rotationDeg,
  shadowRotationDeg,
  active,
}: NavBarRadialButtonProps) {
        ...
        <defs>
          ...
          >
            ...
            <feBlend
              ...
              result="effect1_dropShadow_4_349"
            />

            <feBlend
              mode="normal"
              in="SourceGraphic"
              in2="effect4_dropShadow_4_349"
              result="shape"
            />
          </filter>
        </defs>
...
export default function NavBarRadial({
  open,
  offsetLeft,
  contents,
  buttonFrequency,
  ...
  let buttonContents = Array.from(contents.entries()).filter(
    ([_, { hideInRadial }]) => !hideInRadial,
  );
  ...
  function onJoystickChange(e: GamepadJoystickChangeEvent) {
    ...
    if (
      i >= 0 &&
      i < buttonContents.length &&
      buttonContents[i][0] !== selected
    ) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {buttonContents.map(
          ([originalIndex, { icon, hideInRadial }], i) =>
            hideInRadial || (
              <NavBarRadialButton
                key={i}
