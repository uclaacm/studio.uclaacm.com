...
const collectionContents = content.map(p => [
	p.split("/")[0],
...
const contentLastModificationDates = Object.fromEntries(await Promise.all(
	Object.entries(collectionContents)
		.map(async ([collection, files]) => {
			return [collection, Object.fromEntries(
				await Promise.all(files.map(async filepath => {
					const stat = await fs.stat(path.join(contentRoot, collection, filepath));
			...
			return (
				`${collection}: [
					${files.map((file, fileIndex) => {
						const filename = normalizePath(path.join(path.dirname(file), path.basename(file, path.extname(file))));
						const filePath = path.join(contentRoot, collection, file);
						const modifiedDate = contentLastModificationDates[collection][file];
						fs.stat
						return `{
