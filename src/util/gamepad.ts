...
export class GamepadAxisChangeEvent extends Event {
	constructor(public axis: Axis, public value: number){
...
export class GamepadJoystickChangeEvent extends Event {
	...
	constructor(public joystick: Joystick, public value: [number, number], epsilon: number){
		super("gamepadjoystickchange");

		this.defined = typeof value[0] === "number" && typeof value[1] === "number";
		this.zero = this.defined && Math.abs(value[0]) < epsilon && Math.abs(value[1]) < epsilon;
		this.theta = this.defined && !this.zero
			? Math.atan2(value[1], value[0])
			: undefined;
		this.radius = this.defined
			? this.zero ? 0 : Math.sqrt(value[0] * value[0] + value[1] * value[1])
			: undefined;
	}
}
...
export class GamepadInput extends EventTarget {
	...
	private joystickMap: Map<Joystick, [number | undefined, number | undefined]> = new Map([
	...
	update(){
		...
		this.gamepad.axes.forEach((value, axis) => {
			...
			if(typeof oldValue !== typeof newValue || oldValue !== undefined && Math.abs(oldValue - newValue) > this.epsilon){
		...
		([Joystick.Left, Joystick.Right] as Joystick[]).forEach(joystick => {
			...
			const newValue: [number, number] = [
				this.axisMap.get(joystick === Joystick.Left ? Axis.LHorizontal : Axis.RHorizontal),
				this.axisMap.get(joystick === Joystick.Left ? Axis.LVertical : Axis.RVertical),
			];
			this.joystickMap.set(joystick, newValue);
			if(
				typeof oldValue[0] !== typeof newValue[0] || typeof oldValue[1] !== typeof newValue[1]
				|| Math.abs(oldValue[0] - newValue[0]) > this.epsilon || Math.abs(oldValue[1] - newValue[1]) > this.epsilon
			){
				this.dispatchEvent(new GamepadJoystickChangeEvent(
					joystick,
					newValue,
					this.epsilon
				));
	...
	getAxis(axis: Axis){
		return this.normalizeAxisValue(axis, this.axisSmoothing(this.gamepad.axes[axis as number]));
	}
	...
	private normalizeAxisValue(axis: Axis, value: number){
		return axis === Axis.LVertical || axis === Axis.RVertical ? this.normalizeYAxisValue(value) : value;
	}
